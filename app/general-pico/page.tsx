...
export default async function GeneralPicoPage() {
  ...
  const ciudadParam = "General Pico"
  const turnos = await fetchTurnos(ciudadParam)

  const turnosGeneralPico = turnos.filter(
    (x) => normalize(x.ciudad) === normalize(ciudadParam)
  )

  const pharmaciesOnDutyNow = turnosGeneralPico.filter(isOnDutyNow)

  const otherPharmacies = turnosGeneralPico.filter((x) => !isOnDutyNow(x))
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="container mx-auto px-4 py-4">
        <nav className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span className="text-foreground font-medium">General Pico</span>
        </nav>
      </div>
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          <div className="space-y-3">
            <h1 className="text-3xl md:text-4xl font-bold text-balance leading-tight">
              Farmacias de turno hoy en General Pico
            </h1>
            <div className="flex items-center gap-2">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-base mt-2">
                      Hoy no hay farmacias de turno registradas para General Pico. Por favor, consultá más tarde o
